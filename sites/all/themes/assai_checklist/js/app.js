/* ============================================================
   Checklist LP Assaí — interações (Drupal 7)
   - render das categorias (data-driven)
   - expandir / retrair
   - marcar produtos (Selecione os produtos)
   - baixar lista personalizada (PNG e PDF)
   - monitoramento via track.php
   ============================================================ */
(function () {
  "use strict";

  /* Drupal.settings é injetado depois dos scripts do .info — ler sempre em runtime */
  function cfg(settings) {
    if (settings && settings.assaiChecklist) {
      return settings.assaiChecklist;
    }
    if (window.Drupal && Drupal.settings && Drupal.settings.assaiChecklist) {
      return Drupal.settings.assaiChecklist;
    }
    return {};
  }

  function guessThemeBase() {
    var scripts = document.getElementsByTagName("script");
    var i, src;
    for (i = 0; i < scripts.length; i++) {
      src = scripts[i].src || "";
      if (src.indexOf("assai_checklist/js/app.js") !== -1) {
  ...
  function getBase(settings) {
    var c = cfg(settings);
    return c.base || guessThemeBase();
  }

  /* Evita duplicar /sites/all/themes/assai_checklist/ quando o path já é absoluto */
  function assetUrl(path, settings) {
    if (!path) {
      return "";
    }
    ...
    if (path.indexOf("sites/all/themes/assai_checklist/") === 0) {
      return "/" + path;
    }
    var base = getBase(settings);
    if (!base) {
      return path;
    }
  ...
  function getTrackUrl(settings) {
    var c = cfg(settings);
    if (c.trackUrl) {
      return c.trackUrl;
    }
    var base = guessThemeBase();
    return base ? base + "track/track.php" : "";
  }
  ...
  function track(payload, settings) {
    var TRACK = getTrackUrl(settings);
    var debug = cfg(settings).debug;
    if (!TRACK) {
      if (debug) console.warn("[assai-track] trackUrl vazio — verifique Drupal.settings.assaiChecklist");
      return;
    }
    var body = JSON.stringify(payload);
    if (debug) console.log("[assai-track] POST", TRACK, payload);

    function onResult(status, text) {
      if (!debug) return;
      if (status >= 200 && status < 300) {
        console.log("[assai-track] OK", status, text);
      } else {
        console.error("[assai-track] falhou", status, text);
      }
    }
    ...
    try {
      if (window.fetch) {
        fetch(TRACK, {
          ...
          .then(function (r) { return r.text().then(function (t) { onResult(r.status, t); }); })
          .catch(function (e) { if (debug) console.error("[assai-track] erro de rede", e); });
      } else if (navigator.sendBeacon) {
        navigator.sendBeacon(TRACK, new Blob([body], { type: "application/json" }));
      }
    } catch (e) {
      if (debug) console.error("[assai-track] exceção", e);
    }
  }
  ...
  function renderCategory(cat, settings) {
    ...
    img.src = assetUrl(cat.image, settings);
    img.alt = cat.name;
    ...
    if (cat.cover) {
      ...
      cover.src = assetUrl(cat.cover, settings);
    ...
    btnPng.addEventListener("click", function () {
      ...
      track({
        ...
        category: cat.name,
        count: items.length,
        items: items.map(function (i) { return i.title; })
      }, settings);
      downloadPNG(cat.name, items);
    });
  ...
  /* ---------- bootstrap (Drupal.behaviors roda após Drupal.settings) ---------- */
  Drupal.behaviors.assaiChecklist = {
    attach: function (context, settings) {
      var root = (context.querySelector && context.querySelector("#categories")) ||
        document.getElementById("categories");
      if (!root || root.getAttribute("data-assai-init") === "1") {
        return;
      }
      root.setAttribute("data-assai-init", "1");

      var base = getBase(settings);
      var c = cfg(settings);
      if (c.debug) {
        console.log("[assai-track] behaviors attach", { base: base, trackUrl: getTrackUrl(settings) });
      }
      if (typeof CATEGORIES === "undefined") {
        return;
      }
      CATEGORIES.forEach(function (cat) {
        root.appendChild(renderCategory(cat, settings));
      });
    }
  };
})();
